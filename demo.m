function debugROICallback()
    ...
    % 创建文件选择按钮和输入框
    % 第一个DICOM文件
    uicontrol('Style', 'text', 'Position', [30 650 100 20], 'String', 'DICOM文件 1:');
    fileEdit1 = uicontrol('Style', 'edit', 'Position', [140 650 300 20], 'String', '', 'Enable', 'off');
    uicontrol('Style', 'pushbutton', 'Position', [450 650 80 20], 'String', '浏览...', ...
        'Callback', @(s,e) browseDICOM(fileEdit1, ax1, 'axes1'));
    
    % 第二个DICOM文件
    uicontrol('Style', 'text', 'Position', [30 620 100 20], 'String', 'DICOM文件 2:');
    fileEdit2 = uicontrol('Style', 'edit', 'Position', [140 620 300 20], 'String', '', 'Enable', 'off');
    uicontrol('Style', 'pushbutton', 'Position', [450 620 80 20], 'String', '浏览...', ...
        'Callback', @(s,e) browseDICOM(fileEdit2, ax2, 'axes2'));
    
    % 第三个DICOM文件
    uicontrol('Style', 'text', 'Position', [30 590 100 20], 'String', 'DICOM文件 3:');
    fileEdit3 = uicontrol('Style', 'edit', 'Position', [140 590 300 20], 'String', '', 'Enable', 'off');
    uicontrol('Style', 'pushbutton', 'Position', [450 590 80 20], 'String', '浏览...', ...
        'Callback', @(s,e) browseDICOM(fileEdit3, ax3, 'axes3'));
    
    % ROI尺寸输入
    uicontrol('Style', 'text', 'Position', [30 550 100 20], 'String', 'ROI尺寸:');
    roiSizeEdit = uicontrol('Style', 'edit', 'Position', [140 550 80 20], 'String', '30');
    ...
    function calculateROIData(~, ~)
        ...
        for i = 1:length(axesNames)
            ...
            % 检查该坐标轴是否有图像和ROI
            if ~isempty(imgData.(axName)) && ~isempty(roiInfo.(axName))
                resultStr = [resultStr, sprintf('【%s】:\n', axName)];
                
                % 遍历该坐标轴的所有ROI
                for j = 1:length(roiInfo.(axName))
                    % 获取ROI坐标
                    x1 = roiInfo.(axName)(j).x1;
                    y1 = roiInfo.(axName)(j).y1;
                    x2 = roiInfo.(axName)(j).x2;
                    y2 = roiInfo.(axName)(j).y2;
                    
                    % 提取ROI区域数据
                    roiData = double(imgData.(axName)(y1:y2, x1:x2));
                    ...
                    roiMin = min(roiData(:));
                    roiMax = max(roiData(:));
                    roiSize = size(roiData);
                    
                    % 添加到结果字符串
                    resultStr = [resultStr, sprintf('  ROI%d: 均值=%.2f, 标准差=%.2f, 范围=[%.0f,%.0f], 尺寸=%dx%d\n', ...
                        j, roiMean, roiStd, roiMin, roiMax, roiSize(1), roiSize(2))];
                    
                    % 将ROI数据存储在应用数据中，供其他函数使用
                    if ~isfield(imgData, 'roiValues')
                        imgData.roiValues = struct();
                    end
                    imgData.roiValues.(sprintf('%s_roi%d', axName, j)) = roiData;
                end
