function debugROICallback()
    ...
    % 创建文件选择按钮和输入框
    % 第一个DICOM文件
    uicontrol('Style', 'text', 'Position', [30 650 150 20], 'String', 'DICOM文件 带Gibbs伪影:');
    ...
    % 第二个DICOM文件
    uicontrol('Style', 'text', 'Position', [30 620 150 20], 'String', 'DICOM文件 Fermi滤波:');
    ...
    % 第三个DICOM文件
    uicontrol('Style', 'text', 'Position', [30 590 150 20], 'String', 'DICOM文件 GibbsClear:');
    ...
    % 添加计算按钮
    uicontrol('Style', 'pushbutton', 'Position', [30 500 100 30], 'String', '计算ROI数据', ...
    ...
    function calculateROIData(~, ~)
        ...
        % 检查该坐标轴是否有图像和ROI
        if ~isempty(imgData.('axes1')) && ~isempty(roiInfo.('axes1'))
            
            % 遍历该坐标轴的所有ROI
            for j = 1:2
                % 获取ROI坐标
                x1{j} = roiInfo.('axes1')(j).x1;
                y1{j} = roiInfo.('axes1')(j).y1;
                x2{j} = roiInfo.('axes1')(j).x2;
                y2{j} = roiInfo.('axes1')(j).y2;
                for i = 1:length(axesNames)
                    axName = axesNames{i};
                    % 提取ROI区域数据
                    roiData{i}{j} = double(imgData.(axName)(y1{j}:y2{j}, x1{j}:x2{j}));
                end
            end
                % 计算ROI统计信息
                % SNR
                roi1_ori_avg = mean(roiData{1}{1}(:));
                roi2_ori_std = std(roiData{1}{2}(:));
                roi1_Fermi_avg = mean(roiData{2}{1}(:));
                roi2_Fermi_std = std(roiData{2}{2}(:));
                roi1_AI_avg = mean(roiData{3}{1}(:));
                roi2_AI_std = std(roiData{3}{2}(:));

                ori_SNR = roi1_ori_avg / roi2_ori_std;
                Fermi_SNR = roi1_Fermi_avg / roi2_Fermi_std;
                AI_SNR = roi1_AI_avg / roi2_AI_std;

                DiffRate_Fermi = abs((Fermi_SNR - ori_SNR) / ori_SNR);
                DiffRate_AI = abs((AI_SNR - ori_SNR) / ori_SNR);
                
                Fermi_str = num2str(DiffRate_Fermi*100, '%.2f');
                Ai_str = num2str(DiffRate_AI*100, '%.2f');
                resultStr = [resultStr, 'Fermi滤波的SNR变化率为：', Fermi_str, '%%','   GibbsClear的SNR变化率为：', Ai_str, '%%','    （越小越好）'];
                
                % Energy
                roi1_Fermi_Energy = sum((roiData{2}{1}(:)-roiData{1}{1}(:)).^2) / sum(roiData{1}{1}(:).^2);
                roi2_Fermi_Energy = sum((roiData{2}{2}(:)-roiData{1}{2}(:)).^2) / sum(roiData{1}{2}(:).^2);
                roi1_AI_Energy = sum((roiData{3}{1}(:)-roiData{1}{1}(:)).^2) / sum(roiData{1}{1}(:).^2);
                roi2_AI_Energy = sum((roiData{3}{2}(:)-roiData{1}{2}(:)).^2) / sum(roiData{1}{2}(:).^2);

                roi1_Fermi_Energy_str = num2str(roi1_Fermi_Energy, '%.6f');
                roi2_Fermi_Energy_str = num2str(roi2_Fermi_Energy, '%.6f');
                roi1_AI_Energy_str = num2str(roi1_AI_Energy, '%.6f');
                roi2_AI_Energy_str = num2str(roi2_AI_Energy, '%.6f');
                resultStr = [resultStr, '\n\n在前景区域：\nFermi滤波的相对能量函数值为：', roi1_Fermi_Energy_str,'   GibbsClear的相对能量函数值为：', roi1_AI_Energy_str,'  （越大越好）'...
                                        '\n在背景区域：\nFermi滤波的相对能量函数值为：', roi2_Fermi_Energy_str,'   GibbsClear的相对能量函数值为：', roi2_AI_Energy_str,'  （越小越好）'];
