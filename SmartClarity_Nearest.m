function debugROICallback()
    ...
    % 第二个DICOM文件
    uicontrol('Style', 'text', 'Position', [30 620 150 20], 'String', 'DICOM文件 低分辨图像:');
    ...
    uicontrol('Style', 'text', 'Position', [30 590 150 20], 'String', '最邻近插值:');
    FactorEdit = uicontrol('Style', 'edit', 'Position', [170 590 80 20], 'String', '2');
    uicontrol('Style', 'pushbutton', 'Position', [480 590 80 20], 'String', '开始计算', ...
        'Callback', @(s,e) Nearest(FactorEdit, ax2, 'axes2'));
    
    % 第三个DICOM文件
    uicontrol('Style', 'text', 'Position', [30 560 150 20], 'String', 'DICOM文件 SmartClarity:');
    fileEdit3 = uicontrol('Style', 'edit', 'Position', [170 560 300 20], 'String', '', 'Enable', 'off');
    uicontrol('Style', 'pushbutton', 'Position', [480 560 80 20], 'String', '浏览...', ...
    ...
    % 结果显示区域
    resultText = uicontrol('Style', 'text', 'Position', [30 50 600 130], ...
        'String', '指标数据将显示在这里', ...
    ...
    function Nearest(FactorEdit, axesHandle, axesName)
        imgData = getappdata(fig, 'imgData');
        % 检查axes1是否有图像
        if isempty(imgData.axes2)
            errordlg('请先加载低分辨参考图', '错误');
            return;
        end
        img = double(imgData.axes2);
        % 获取缩放因子
        scaleFactor = str2double(get(FactorEdit, 'String'));
        ...
        % 最邻近插值核心算法
        [rows, cols] = size(img);                  % 获取原始图像尺寸
        newRows = round(rows * scaleFactor);       % 计算新图像高度
        newCols = round(cols * scaleFactor);       % 计算新图像宽度
        superResImg = zeros(newRows, newCols);     % 初始化超分图像
        
        % 最邻近插值计算
        for i = 1:newRows
            % 计算原始图像中对应的行索引（四舍五入取最近邻）
            originalRow = round(i / scaleFactor);
            % 防止索引越界
            originalRow = max(1, min(originalRow, rows));
            
            for j = 1:newCols
                % 计算原始图像中对应的列索引（四舍五入取最近邻）
                originalCol = round(j / scaleFactor);
                % 防止索引越界
                originalCol = max(1, min(originalCol, cols));
                
                % 赋值最邻近像素值
                superResImg(i, j) = img(originalRow, originalCol);
            end
        end
        
        % 更新图像数据
        imgData.(axesName) = superResImg;
        setappdata(fig, 'imgData', imgData);
        ...
        % 显示图像
        himg = imshow(superResImg, [], 'Parent', axesHandle);
        ...
        % 更新标题
        title(axesHandle, sprintf('最邻近插值 (缩放因子: %.1f)', scaleFactor));
        ...
        fprintf('已生成最邻近插值图像到 %s，尺寸: %dx%d\n', axesName, newRows, newCols);
    end
    ...
    % end
    % 
    function cp=PSNR(im_rec,im_ori)
    ...
    function calculateROIData(~, ~)
        ...
        high_rever = abs(image{1});
        high_rever_norm = high_rever ./ max(high_rever(:));
        high_Cubic = abs(image{2});
        high_Cubic_norm = high_Cubic ./ max(high_Cubic(:));
        high_SmartClarity = abs(image{3});
        high_SmartClarity_norm = high_SmartClarity ./ max(high_SmartClarity(:));

        psnr_Cubic = PSNR(high_Cubic_norm, high_rever_norm);
        psnr_SmartClarity = PSNR(high_SmartClarity_norm, high_rever_norm);

        ssim_Cubic = ssim(high_rever_norm, high_Cubic_norm);
        ssim_SmartClarity = ssim(high_rever_norm, high_SmartClarity_norm);

        psnr_Cubic_str = num2str(psnr_Cubic, '%.4f');
        psnr_SmartClarity_str = num2str(psnr_SmartClarity, '%.4f');
        ssim_Cubic_str = num2str(ssim_Cubic, '%.4f');
        ssim_SmartClarity_str = num2str(ssim_SmartClarity, '%.4f');
