function debugROICallback()
    ...
    % 创建文件选择按钮和输入框
    % 第一个DICOM文件
    uicontrol('Style', 'text', 'Position', [30 650 100 20], 'String', 'DICOM文件 去噪前:');
    ...
    % 第二个DICOM文件
    uicontrol('Style', 'text', 'Position', [30 620 100 20], 'String', 'DICOM文件 去噪后:');
    ...
    function calculateROIData(~, ~)
        ...
        if ~isempty(imgData.('axes1')) && ~isempty(roiInfo.('axes1'))
            ...
            for j = 1:2
                ...
                % 计算ROI统计信息
                % SNR
                roi1_noisy_avg = mean(roiData{1}{1}(:));
                roi2_noisy_std = std(roiData{1}{2}(:));
                roi1_denoisy_avg = mean(roiData{2}{1}(:));
                roi2_denoisy_std = std(roiData{2}{2}(:));

                noisy_SNR = roi1_noisy_avg / roi2_noisy_std;
                denoisy_SNR = roi1_denoisy_avg / roi2_denoisy_std;
                
                noisy_SNR_str = num2str(noisy_SNR, '%.2f');
                denoisy_SNR_str = num2str(denoisy_SNR, '%.2f');
                resultStr = [resultStr, '去噪前的SNR为：', noisy_SNR_str, '   iDEAL去噪后的SNR为：', denoisy_SNR_str,'    （越大越好）'];
